...
In this example, we want to pull the latest data when the user enters each category, such as pulling and storing movies *only* when the user enters the movies category. We also want this data to be cached, because the user will likely be scrolling through movies and entering sub-screens that can use the data we've already pulled from the network. This will speed up the loading of each sub-screen, while also minimizing network usage.
